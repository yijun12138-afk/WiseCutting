...
public interface FabricSpreadDao extends BaseMapper<FabricSpreadEntity> {
    ...
    @Select("SELECT COUNT(*) FROM t_fabric_spread WHERE deleted_flag = 0 AND create_time BETWEEN #{start} AND #{end}")
    Long countByCreateTimeBetween(@Param("start") LocalDateTime start, @Param("end") LocalDateTime end);

    @Select("SELECT COUNT(*) FROM t_fabric_spread WHERE deleted_flag = 0 AND status = 2 AND update_time BETWEEN #{start} AND #{end}")
