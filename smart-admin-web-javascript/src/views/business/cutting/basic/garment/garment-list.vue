<template>
  ...
  <a-card size="small" :bordered="false" :hoverable="true">
    ...
    <GarmentFormModal ref="formModal" @reload="queryData" />
  </a-card>
</template>
<script setup>
...
import GarmentFormModal from './components/garment-form-modal.vue';
...
function onToggleDisabled(row) {
  ...
  Modal.confirm({
    title: '提示', content: `确定${action}成衣【${row.garmentName}】吗?`, okText: action, okType: 'primary',
    onOk: async () => {
      try { SmartLoading.show(); await garmentApi.updateDisabledFlag(row.garmentId, !row.disabledFlag); message.success(`${action}成功`); queryData(); }
      catch (e) { smartSentry.captureError(e); } finally { SmartLoading.hide(); }
    },
  });
}
function onDelete(row) {
  Modal.confirm({ title: '提示', content: `确定删除【${row.garmentName}】吗?`, okText: '删除', okType: 'danger',
    onOk: async () => { try { SmartLoading.show(); await garmentApi.delete(row.garmentId); message.success('删除成功'); queryData(); } catch (e) { smartSentry.captureError(e); } finally { SmartLoading.hide(); } } });
}
</script>
